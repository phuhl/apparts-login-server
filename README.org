...
2. Add the package to your codebase:
   #+BEGIN_SRC js
     const { addRoutes, useUser } = require("@apparts/login-server");
   ...
   - If you need to configure the User-model:
     - The table name (default ~users~):
       #+BEGIN_SRC js
         const { useUser: _useUser } = require("@apparts/login-server");
         const useUser = (dbs) => _useUser(dbs, undefined, "myTableName");
       #+END_SRC
     - The types of the user (see [[https://github.com/phuhl/apparts-types#usage][@apparts/types]]):
       #+BEGIN_SRC js
         const { useUser: _useUser } = require("@apparts/login-server");
         const useUser = (dbs) => _useUser(dbs, { customElement: { type: "string", public: true }});
       #+END_SRC
...
*** Request password reset: POST =/v1/user/:email/password=
